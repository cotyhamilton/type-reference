document.getElementById("search-field").addEventListener("keyup", autocomplete);
...
function autocomplete() {
    ...
    if (x.value) {
        pokemonNameList.forEach(function(name) {
            if (name.indexOf(x.value.toLowerCase()) == 0) {
                results.push(name);
            }
        });

        console.log(results);
        document.getElementById('box').appendChild(makeUL(results));
        if (!results.length) {document.getElementById('box').innerHTML = "";};
    }
